...
// Create transporter
const createTransporter = () => {  // Check if SMTP credentials are configured
...
const sendDonationConfirmation = async (donation) => {
  ...
  if (!transporter) {
    console.log('Skipping donation confirmation email - SMTP not configured');
    return;
  }
...
const sendContactConfirmation = async (contact) => {
  const transporter = createTransporter();
  if (!transporter) {
    console.log('Skipping contact confirmation email - SMTP not configured');
    return;
  }
...
const notifyAdminNewContact = async (contact) => {
  const transporter = createTransporter();
  if (!transporter) {
    console.log('Skipping admin notification email - SMTP not configured');
    return;
  }
  ...
  const mailOptions = {
    ...
    subject: `New Contact Form Submission - ${escapeHtml(contact.subject)}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
